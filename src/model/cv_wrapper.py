...
def cross_val_run(x_num, x_cat, y,
                  build_model_fn, hyper, n_splits=5, seed=100):
    ...
    for fold, (idx_tr, idx_va) in enumerate(skf.split(X_dummy, y_np)):
        ...
        bs      = hyper["batch_size"]
        scope   = ScopeClassifier(model, hyper,
                                  steps_per_epoch=len(idx_tr)//bs)
        ...
        trainer.set_data(
            (x_num[idx_tr], x_cat[idx_tr]), y[idx_tr],
            (x_num[idx_va], x_cat[idx_va]), y[idx_va]
        )
